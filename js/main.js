...
app.post("/admin_signup_search", (req, res)=>{ // 일부 검색(회원가입 대기 목록 검색)
...
app.post("/admin_signup_recept", (req, res)=>{ // 회원가입 신청 수락(-> 회원으로 등록)
...
app.post("/admin_rentalmanage_resrv_reject", (req, res)=>{ // 회원가입 신청 거절(-> DB에서 삭제)
...
app.get("/admin_signup_rewrite", (request, response)=>{ // 회원가입 신청 폼 수정(회원으로 등록 전 DB에서 사용자 정보 확인&수정)
    if (request.session.user_auth==2) {
